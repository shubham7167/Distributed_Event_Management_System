	...
	if(Manager.contains(ManagerID.trim()))
	  {
        ...
        else
        {
			...
			msg="ENTER VALID BOOKING CAPACITY.";
			}
	  }
	else
	 { 
			  ...
			  if(customerID.substring(0,4).equalsIgnoreCase("MTLC"))
			  {
				...
				else if(eventType.equalsIgnoreCase("TS")) 
				{
					...
					if(alreadyPresent(customerID,eventID,eventType)){
						...
						System.out.println("in sem");
						System.out.println(eventType);
						System.out.println(MTLEvents);
						System.out.println(MTLEvents.get(eventType.trim()));
						if(MTLEvents.get(eventType).containsKey(eventID)) {
							System.out.println("in sem");
							int eve=MTLEvents.get(eventType).get(eventID);
	       	  ...
	       	  while(true)
	       	  {
	       		  ...
	       		  try
	       		  {
	    					...
	    					if(method.equals("BOOK"))
	    					{
	    						...
	    						System.out.println("customerID:"+id+"eventID:"+eventID+" eventType:"+eventType);
	    					...
	    					else if(method.trim().equals("GETSCHEDULE"))
	    					{
	    						...
	    						if(ms.flag1==0)
	    						{
	    							System.out.println(ms.flag1);
	    							reply=ms.getBookingSchedule(id.trim());
	    							System.out.println(reply);
	    							InetAddress ip1=InetAddress.getByName("132.205.46.142");
	    							dp1=new DatagramPacket("1".getBytes(),"1".length(),ip1,7001);
	    							ds1.send(dp1);
   						}
	    						else
    						{
	       	 ...
	       	 Runnable task2 =()->
	         {
	        	 ...
	        	 while(true)
	        	 {
	        		 ...
	        		 try
	        		 {
	    				...
	    				ds2.receive(dp2);
	    				byte[] msg=dp2.getData();
	    				System.out.println("Messsage from RM: "+ msg.toString());
	    				ms.flag1=1;
	        		 }
	        		 catch(Exception e)
	        		 {
